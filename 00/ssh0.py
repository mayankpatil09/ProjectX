...
def connect_to_server(host, port, username, password, keyfile_path=None):
    ...
    try:
        if keyfile_path and os.path.exists(keyfile_path):
            ...
            ssh.connect(host, port, username=username, pkey=private_key)
        else:
...
def transfer_folder(ssh, local_path, remote_folder, progress_callback=None):
    ...
    if os.path.isdir(local_path):
        ...
        rename_command = f'Rename-Item -Path "{remote_path}" -NewName "{new_folder_name}"'
        stdin, stdout, stderr = ssh.exec_command(f'powershell -Command "{rename_command}"')
        error = stderr.read().decode().strip()
